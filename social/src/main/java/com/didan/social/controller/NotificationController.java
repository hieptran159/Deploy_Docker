...
public class NotificationController {
    ...
    @Operation(summary = "Mark all my unread notifications of a type as read",
            security = @SecurityRequirement(name = "bearerAuth"))
    @PatchMapping("/type/{type}/read")
    public ResponseEntity<?> readByType(@PathVariable("type") String type) {
        ResponseData payload = new ResponseData();
        try {
            Map<String, Object> data = new HashMap<>();
            data.put("marked", notificationService.markReadByType(type));
            payload.setData(data);
            payload.setDescription("Marked by type");
            return new ResponseEntity<>(payload, HttpStatus.OK);
        } catch (Exception e) {
            payload.setSuccess(false);
            payload.setStatusCode(500);
